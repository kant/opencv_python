...
def get_image_sequence(image_folder):
  ...
  while (cv2.waitKey() & 0xff) != ord('q'):  # Press 'q' to quit the loop
    ...
    if flag_first_img == True:
      ...
      row = ground_truth_array[0]
      print('row', row)
      prev_left_pan = float(row[8])
      prev_left_tilt = float(row[9])
      prev_right_pan = float(row[10])
      prev_right_tilt = float(row[11])
      print('first', prev_left_pan, prev_left_tilt, prev_right_pan, prev_right_tilt)
    else:
...
def get_image_ground_truth(image_folder):
  ...
  original_filename += '/groundtruth_with_ptu.txt'
